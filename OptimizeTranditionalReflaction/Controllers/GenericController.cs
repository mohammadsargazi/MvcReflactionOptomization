...
public partial class GenericController<T, T1> : BaseController<T1> where T : BaseEntity where T1 : DbContext, ICustomDbContext
{
    ...
    public GenericController(T1 context, IConfiguration config, IWebHostEnvironment webHostEnvironment) : base(context, config, webHostEnvironment)
    {
        ...
        _cachedProperty = typeof(T).GetProperties(BindingFlags.Public | BindingFlags.Instance).ToList();
    ...
    {
        var allp = type.GetProperties().ToList();
        allp = allp.Where(i => i.PropertyType.Name.StartsWith("ICollection") || i.CustomAttributes.Any(k => k.AttributeType == typeof(ForeignKeyAttribute))).ToList();
